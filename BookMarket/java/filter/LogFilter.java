...
public class LogFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		// 요청과 응답에 대한 로그 기록
		System.out.println("접속한 클리아언트 ip: " + request.getRemoteAddr());
		long start = System.currentTimeMillis(); // 시작시간
		System.out.println("접근한 URL 경로 (커스텀) :" + getURLpath(request));
		// 요청받은 주소가 내가 보고싶은 메서드로 변환되어 콘솔 출력
		System.out.println("요청 처이 시작 시간 (커스텀) :" + getCurrentTime());
		chain.doFilter(request, response);// 필터에게 요청과 응답에 대한 자료를 전송한다.

		long end = System.currentTimeMillis(); // 종료 시간 저장용
		System.out.println("요청처리 종료시간 (커스텀) : " + getCurrentTime());
		System.out.println("요청처리 소요 시간 (시작-종료) :" + (end - start) + "ms");
